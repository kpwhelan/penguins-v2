    ...
    const submit = (event) => {
        ...
        axios.post(route('swimmer-bios.store'), formData, { headers: { 'Content-Type': 'multipart/form-data' } })
            ...
            .catch((error) => {
                ...
                toast.error(error.response?.data?.message ?? 'The Penguins All Star could not be published.');
            })
    ...
    return (
        <form onSubmit={submit} id="swimmer-bio-form" className={`overflow-hidden rounded-panel border border-navy-950/15 bg-white shadow-card ${className}`}>
            ...
            <header className="flex items-start gap-4 border-b border-navy-950/10 p-6 sm:p-7">
                ...
                <div>
                    <p className="text-xs font-extrabold uppercase tracking-[0.15em] text-penguins-700">Penguins All Stars</p>
                    <h3 className="mt-1 text-xl font-extrabold text-navy-950">Feature a Penguins All Star</h3>
                    <p className="mt-2 text-sm leading-6 text-slate">Celebrate a teammate and share their story on the public site.</p>
                </div>
            </header>

            <div className="space-y-5 p-6 sm:p-7">
                <div>
                    <InputLabel htmlFor="swimmer_image" value="All Star photo" />
                    <label htmlFor="swimmer_image" className="mt-2 flex min-h-24 cursor-pointer items-center gap-4 rounded-2xl border border-dashed border-navy-950/25 bg-mist px-5 py-4 transition hover:border-penguins-600 hover:bg-penguins-50">
                        ...
                        <span className="min-w-0"><span className="block truncate text-sm font-extrabold text-navy-950">{fileData?.name ?? 'Choose a portrait'}</span><span className="mt-1 block text-xs text-slate">JPG, PNG, or WebP · optimized automatically</span></span>
                ...
                <div>
                    <InputLabel htmlFor="swimmer_name" value="All Star name" />
                ...
                <div>
                    <div className="flex items-center justify-between gap-4"><InputLabel htmlFor="swimmer_body" value="Their story" /><span className="text-xs text-slate">{data.body.length} characters</span></div>
            ...
            <footer className="flex items-center justify-between gap-4 border-t border-navy-950/10 bg-navy-50/60 px-6 py-5 sm:px-7">
                <p className="text-xs leading-5 text-slate">Appears in the Penguins All Stars section.</p>
                <PrimaryButton disabled={submitting}>{submitting ? 'Publishing…' : 'Publish All Star'}</PrimaryButton>
            </footer>
        </form>
    );
}
